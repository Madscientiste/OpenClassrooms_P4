...
def findby_id(player_id, context, actions):
    ...
    if not player_id or not player_id.isdigit():
        context["error_view"].generic_error(message="id must be a number", centered=True)
        return 0

    found_player = context["player_model"].find_one(id=int(player_id))

    if not found_player:
        context["error_view"].generic_error(message=f"player id [{player_id}] not found", centered=True)
        return 0
...
def findby_name(player_name, context, actions):
    ...
    if not found_players:
        context["error_view"].generic_error(message=f"No players with name of [{player_name}] found", centered=True)
        return 0
...
def findby_rank(player_rank, context, actions):
    ...
    if not player_rank or not player_rank.isdigit():
        context["error_view"].generic_error(message="rank must be a number", centered=True)
        return 0

    found_players = context["player_model"].find_many("rank", player_rank)

    if not found_players:
        context["error_view"].generic_error(message=f"player with [{player_rank}] rank not found", centered=True)
        return 0
